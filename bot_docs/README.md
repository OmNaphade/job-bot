...
- **[SOURCES.md](SOURCES.md)** — how to turn on each job source (Telegram, LinkedIn/Naukri via email, RSS/JSON feeds, direct-from-company-site via Greenhouse/Lever/Ashby, and the reverse-engineered Foundit/Unstop integrations — read the risk assessment there before enabling those two), which sources were evaluated and rejected (and why), and the full environment variable reference.
